...
// source: http://codereview.stackexchange.com/questions/40473/portable-periodic-one-shot-timer-implementation
class Timer
{
public:
	typedef uint64_t timer_id;
	static const timer_id EmptyTimer;
...
private:
	...
	struct Instance
	{
		...
		Instance(timer_id id, Timestamp next, Duration period, Tfunction&& handler)
			: id(id)
			, next(next)
			, period(period)
